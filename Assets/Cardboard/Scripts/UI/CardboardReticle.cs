...
public class CardboardReticle : MonoBehaviour, ICardboardGazePointer {
  ...
  private const float kReticleDistanceMax = 20.0f;
  ...
  public void OnGazeStart(Camera camera, GameObject targetObject, Vector3 intersectionPosition,
		...
		if (targetObject.tag == "Property" && !ObjectController.instance.GetMove() && !ObjectController.instance.checkPreviewStatus ()) {
		if (targetObj != null) 
			ObjectController.instance.ReleaseTarget ();
			
		targetObj = targetObject;
		
		ObjectController.instance.SetTarget (targetObj);
	}

  	iPosition = intersectionPosition;
  ...
  public void OnGazeStay(Camera camera, GameObject targetObject, Vector3 intersectionPosition,
                         bool isInteractive) {
//
//	if (targetObject.tag == "Property" && ObjectController.instance.GetTarget () == null) {
//		targetObj = targetObject;
//	}
  ...
  public void OnGazeExit(Camera camera, GameObject targetObject) {
	if (targetObj != null && !ObjectController.instance.GetMove()) {
		ObjectController.instance.ReleaseTarget ();
		targetObj = null;
	}
